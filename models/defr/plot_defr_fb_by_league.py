#!/usr/bin/env python3
"""Per-league fullback DefR charts (StatsBomb-article style), one each
for Liga 3 and Campeonato. Fullbacks (LB/RB), >=700 min. Labels the ~28
most interesting players (outliers + DefR/demand/volume extremes).
Run from the Dashboard dir: python models/defr/plot_defr_fb_by_league.py
Outputs: plots/defr_fb_liga3_2526.png, plots/defr_fb_camp_2526.png"""
...
def make(sid, label, color, fname):
    cur=df[df['position'].isin(['LB','RB']) & (df['seasonId']==sid) & (df['mins_played']>=700)].copy()
    ...
    ax.set_title(f'Fullback Defensive Responsibility — {label} 25/26',fontsize=16,fontweight='bold',color=C_TXT,pad=14)
    fig.text(0.5,0.915,f'Fullbacks (LB/RB), ≥700 min ({len(cur)} players, {len(lab)} labelled). Above the dashed line = more than a typical fullback in the same situation.',ha='center',fontsize=10.5,color='#5f6368')
    plt.tight_layout(rect=[0,0,1,0.93])
    out=str(_HERE/'plots'/fname)
    plt.savefig(out,dpi=140,bbox_inches='tight',facecolor='white'); plt.close()
    print(f"Saved {out}  ({len(cur)} FBs, {len(lab)} labelled)")

make(191782,'Liga 3','#2E6FB0','defr_fb_liga3_2526.png')
make(191779,'Campeonato','#E08A2B','defr_fb_camp_2526.png')
